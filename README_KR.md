...
| 모델 | 설명 |
|---|---|
| OLS ADL (lag-2) | AR 2기, 설명변수 1기 시차, HC3 강건 표준오차 |
| OLS ADL (lag-3) | AR 3기, 설명변수 2기 시차로 확장 |
| ARIMAX | AIC 기준 (p, q) grid search; 이미 차분된 데이터라 d=0 고정 |

2008 금융위기와 2020 코로나 구간은 `is_crisis` 더미로 처리 — 두 구간 모두 lag만으로는 잡기 어려운 구조적 변화다.

### 성능 결과

세 모델 모두 naive baseline(변화 없음 예측)을 ~7–9% 이겼다:

| 모델 | RMSE | MAE |
|---|---|---|
| Naive (변화 없음) | 0.9095 | 0.7532 |
| OLS (lag-2) | 0.8540 | 0.6847 |
| OLS (lag-3) | 0.8485 | 0.6878 |
| ARIMAX | 0.8421 | 0.6853 |

![OLS vs ARIMAX: 실제값 vs 예측값](data/img/OLS%20vs%20ARIMAX.png)

세 모델 성능이 거의 동일한 이유는 테스트 구간(2022–) 자체가 훈련 데이터에서 본 적 없는 regime이기 때문이다 — 팬데믹 이후 정상화 + 급격한 금리 인상.

### 핵심 발견: 계수 분석

통계적으로 유의하고 실질적으로 의미 있는 변수는 `unrate`와 `unrate_lag1` 뿐이었다:

1. **`unrate` 계수 ~1.8**: 전체 실업률이 1pp 오르면 청년실업률은 1.8pp 오른다. 단순한 동조가 아니라 충격이 청년층에 증폭된다.
2. **`fed_funds`, `cpi` 무의미**: 금리 인상이 청년실업에 직접 영향을 주지 않는다. 통화정책은 전체 고용시장을 통해서만 간접적으로 작용한다.
3. **`unrate_lag1` 음수**: 전체 실업률이 급등한 다음 달에 청년실업이 소폭 되돌아오는 패턴 — 초기 충격이 과도하게 반응했다가 조정된다.

결론: **청년실업은 금리나 물가에 독립적으로 반응하는 게 아니라, 전체 노동시장 흐름에 구조적으로 종속되어 있다.**
